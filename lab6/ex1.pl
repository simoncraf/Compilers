...
encodetest(test(Op,A1,A2),D,L, (Ecode; instr(J,L))):-
    encodeexpr(expr(_,A1,A2),D,Ecode),
    newop(Op,Jumpif).
...
compile(Source, (Code; instr(halt,0); block(L)) ) :-
 ...
 %thw code to assemble is a sequence of things
 assemble((Code1,Code2),N0,N):-
    assemble(Code1,N0,N1),
    assemble(Code2,N1,N).

%the code to assemble is a single instruction
assemble(instr(_,_),N0,N):- N is N0+1.

%the code to assemble is a label
